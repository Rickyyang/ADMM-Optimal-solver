%% combine multiple inequality constraints using logic And
% logic or : D1(x)>0 and  D2(x)>0 and  D3(x)>0  = min(D(x))>0
%    
function [D,dD] = combineDxFcnWithLogicAnd(ineqConstraint,ineqConstraint_grad)
...
for j = 1:nConstraint
    ...
    function f = generateConstrainFcn(x)
        ...
        constraintsStatus = not(all(constraintStatus,2));
