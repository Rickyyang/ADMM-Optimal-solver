...
function [f,df] = trajectory_cost(field_prop,varargin)
% need trajectory length for multi agent case
ivarargin = 1;
n_agent = 1;
if ivarargin<=length(varargin)
    traj_length=varargin{ivarargin};
    n_agent = max([field_prop.agent_id]);
end

if n_agent == 1
    % single agent case
    df = @(x) der_full_cost(x,field_prop);
